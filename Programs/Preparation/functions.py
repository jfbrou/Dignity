...
def v_of_ell(x, epsilon=1.0, theta=8.851015121158213):
...
# Define the level consumption-equivalent welfare calculation function
def cew_level_gamma(S_i=None, S_j=None, I_i=None, I_j=None, Eu_of_c_and_ell_i=None, Eu_of_c_and_ell_j=None, age_min=0, age_max=100, gamma=2, epsilon=1, theta=8.851015121158213, # Standard parameters
    ...
    def u(c, ell, gamma=gamma, epsilon=1, theta=8.851015121158213):
    ...
    def du_dc(c, ell, gamma=gamma, epsilon=1, theta=8.851015121158213):
...
# Define the level consumption-equivalent welfare calculation function with the morbidity adjustment
def cew_level_morbidity(S_i=None, S_j=None, I_i=None, I_j=None, H_i=None, H_j=None, c_i_bar=None, c_j_bar=None, ell_i_bar=None, ell_j_bar=None, age_min=0, age_max=100, # Standard parameters
                        S_intercept=None, I_intercept=None, H_intercept=None, c_intercept=None, ell_intercept=None, vsl=7.4e6, c_nominal=None, age_min_intercept=40, age_max_intercept=100, # Intercept parameters
                        c_i_bar_nd=None, c_j_bar_nd=None, Elog_of_c_i=None, Elog_of_c_j=None, Elog_of_c_i_nd=None, Elog_of_c_j_nd=None, Ev_of_ell_i=None, Ev_of_ell_j=None, # Inequality parameters
                        morbidity_parameter=None): # Morbidity adjustment parameters
    # Restrict on selected ages
    S_i = S_i[age_min:age_max + 1] / S_i[age_min]
    S_j = S_j[age_min:age_max + 1] / S_j[age_min]
    I_i = I_i[age_min:age_max + 1]
    I_j = I_j[age_min:age_max + 1]
    H_i = H_i[age_min:age_max + 1]
    H_j = H_j[age_min:age_max + 1]
    c_i_bar = c_i_bar[age_min:age_max + 1]
    c_j_bar = c_j_bar[age_min:age_max + 1]
    ell_i_bar = ell_i_bar[age_min:age_max + 1]
    ell_j_bar = ell_j_bar[age_min:age_max + 1]
    ...
    S_intercept = S_intercept[age_min_intercept:age_max_intercept + 1] / S_intercept[age_min_intercept]
    I_intercept = I_intercept[age_min_intercept:age_max_intercept + 1]
    H_intercept = H_intercept[age_min_intercept:age_max_intercept + 1]
    c_intercept = c_intercept[age_min_intercept:age_max_intercept + 1]
    ell_intercept = ell_intercept[age_min_intercept:age_max_intercept + 1]

    # Calculate the quality of life terms from the HALex
    Q_i = morbidity_parameter + (1 - morbidity_parameter) * H_i
    Q_j = morbidity_parameter + (1 - morbidity_parameter) * H_j
    Q_intercept = morbidity_parameter + (1 - morbidity_parameter) * H_intercept

    # Calculate the intercept
    u_bar = (vsl / c_nominal - dot(S_intercept * (1 - I_intercept) * Q_intercept, log(c_intercept) + v_of_ell(ell_intercept))) / sum(S_intercept * (1 - I_intercept) * Q_intercept)

    # Define the lower case survival rates
    s_i = S_i * (1 - I_i) * Q_i / sum(S_i * (1 - I_i) * Q_i)
    s_j = S_j * (1 - I_j) * Q_j / sum(S_j * (1 - I_j) * Q_j)
    Delta_s_EV = (S_j - S_i) * (1 - I_j) * Q_j / sum(S_i * (1 - I_i) * Q_i)
    Delta_s_CV = (S_j - S_i) * (1 - I_i) * Q_i / sum(S_j * (1 - I_j) * Q_j)
    Delta_i_EV = (I_i - I_j) * S_i * Q_i / sum(S_i * (1 - I_i) * Q_i)
    Delta_i_CV = (I_i - I_j) * S_j * Q_j / sum(S_j * (1 - I_j) * Q_j)
    Delta_q_EV = (Q_j - Q_i) * S_i * (1 - I_j) / sum(S_i * (1 - I_i) * Q_i)
    Delta_q_CV = (Q_j - Q_i) * S_j * (1 - I_i) / sum(S_j * (1 - I_j) * Q_j)
    ...
    # Calculate the EV and CV consumption-equivalent welfare, and average them
    log_lambda_EV = LE_EV + I_EV + M_EV + C_EV + CI_EV + L_EV + LI_EV
    log_lambda_CV = LE_CV + I_CV + M_CV + C_CV + CI_CV + L_CV + LI_CV
    log_lambda = (log_lambda_EV + log_lambda_CV) / 2
