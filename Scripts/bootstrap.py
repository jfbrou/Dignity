...
def cew(b):
    ...
    df_bootstrap = dignity_bootstrap.loc[dignity_bootstrap.year.isin(years) & (dignity_bootstrap.bootstrap == b) & (dignity_bootstrap.race != -1) & (dignity_bootstrap.latin == -1), :]
    ...
    df = expand({'year': years, 'race': [2], 'log_lambda': [np.nan], 'bootstrap': [b], 'description': ['black']})
    for year in years:
    ...
    df_bootstrap = dignity_bootstrap.loc[dignity_bootstrap.year.isin(years) & (dignity_bootstrap.bootstrap == b) & (dignity_bootstrap.race != -1) & (dignity_bootstrap.latin == 0), :]
    ...
    df_black = expand({'year': years, 'race': [2], 'log_lambda': [np.nan], 'bootstrap': [b], 'description': ['black non-latino']})
    for year in years:
        S_i = df_survival.loc[(df_survival.year == year) & (df_survival.race == 1), 'S'].values
        S_j = df_survival.loc[(df_survival.year == year) & (df_survival.race == 2), 'S'].values
        c_i_bar = df_bootstrap.loc[(df_bootstrap.year == year) & (df_bootstrap.race == 1), 'c_bar'].values
    ...
    years = range(2006, 2019 + 1)
    df_bootstrap = dignity_bootstrap.loc[dignity_bootstrap.year.isin(years) & (dignity_bootstrap.bootstrap == b), :]
    ...
    df_latin = expand({'year': years, 'race': [-1], 'log_lambda': [np.nan], 'bootstrap': [b], 'description': ['latino']})
    for year in years:
    ...
    df_growth = expand({'year': years[1:], 'race': [1, 2], 'log_lambda': [np.nan], 'bootstrap': [b], 'description': ['growth']})
    for race in [1, 2]:
        for year in years[1:]:
