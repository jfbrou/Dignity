...
################################################################################
#                                                                              #
# This section of the script calculates the COVID-19 mortality rates by        #
# race and age from April 2020 to April 2021.                                  #
#                                                                              #
################################################################################
...
covid = covid.loc[(covid.date >= start_date) & (covid.date <= end_date), :].drop(['mmwryear', 'mmwrweek', 'day', 'date'], axis=1)
...
#                                                                              #
# This section of the script calculates the COVID-19 mortality rates by        #
# race, gender, age and month with the most recent data in 2020.               #
#                                                                              #
################################################################################

# Retrieve the weekly COVID-19 mortality data
client = Socrata('data.cdc.gov', os.getenv('cdc_api_key'))
covid_monthly = pd.DataFrame.from_records(client.get('m74n-4hbs', limit=500000)).loc[:, ['mmwryear', 'mmwrweek', 'raceethnicity', 'sex', 'agegroup', 'covid19_weighted']]

# Find the first day of each week
covid_monthly.loc[:, 'day'] = 7 * covid_monthly.mmwrweek.astype('int') - 6

# Find the date of each observation
covid_monthly.loc[:, 'date'] = pd.to_datetime(covid_monthly.mmwryear.astype(str), format='%Y') + pd.to_timedelta(covid_monthly.day.astype(str) + 'days')

# Find the month of each observation
covid_monthly.loc[:, 'month'] = covid_monthly.date.dt.month

# Keep the year covid-19 death counts from April 2020 to April 2021
start_date = pd.to_datetime('2020-04-01')
end_date = pd.to_datetime('2021-04-01')
covid_monthly = covid_monthly.loc[(covid_monthly.date >= start_date) & (covid_monthly.date <= end_date), :].drop(['mmwryear', 'mmwrweek', 'day', 'date'], axis=1)

# Recode the race variable
covid_monthly = covid_monthly.loc[covid_monthly.raceethnicity.isin(['Non-Hispanic White', 'Non-Hispanic Black', 'Hispanic', 'All Race/Ethnicity Groups']), :]
covid_monthly.loc[:, 'latin'] = covid_monthly.raceethnicity.map({'Non-Hispanic White': 0, 'Non-Hispanic Black': 0, 'Hispanic': 1, 'All Race/Ethnicity Groups': -1})
covid_monthly.loc[:, 'raceethnicity'] = covid_monthly.raceethnicity.map({'Non-Hispanic White': 1, 'Non-Hispanic Black': 2, 'Hispanic': -1, 'All Race/Ethnicity Groups': -1})

# Recode the gender variable
covid_monthly = covid_monthly.loc[covid_monthly.sex == 'All Sexes', :].drop('sex', axis=1)

# Drop the aggregate and missing age categories and recode the age variable
covid_monthly = covid_monthly.loc[(covid_monthly.agegroup != 'All Ages') & (covid_monthly.agegroup != 'Not stated'), :]
covid_monthly.loc[:, 'agegroup'] = covid_monthly.agegroup.map({'0-14 Years': 1,
                                                        '15-19 Years': 2,
                                                        '20-24 Years': 3,
                                                        '25-29 Years': 4,
                                                        '30-34 Years': 5,
...
# Rename variables and define their type
covid_monthly = covid_monthly.rename(columns={'raceethnicity':    'race',
                                              'covid19_weighted': 'deaths'})
covid_monthly = covid_monthly.astype({'month':    'int',
                                      'race':     'int',
                                      'latin':    'int',
                                      'agegroup': 'int',
                                      'deaths':   'int'})

# Sum the COVID-19 deaths by group and month
covid_monthly = covid_monthly.groupby(['race', 'latin', 'agegroup', 'month'], as_index=False).agg({'deaths': 'sum'})
...
# Merge the two data frames
df_group = pd.merge(df_group, covid_monthly, how='left')

# Compute the COVID-19 mortality rate by race and age group
df_group.loc[:, 'M'] = df_group.deaths / df_group.population
df_group.loc[df_group.M == 0, 'M'] = 1e-10

# Create a data frame with all levels of all variables
df_covid = expand({'month': df_group.month.unique(), 'race': df_group.race.unique(), 'latin': df_group.latin.unique(), 'age': range(100)})
df_covid = df_covid.loc[(df_covid.race.isin([1, 2]) & (df_covid.latin == 0)) | ((df_covid.race == -1) & df_covid.latin.isin([-1, 1])), :]

# Define a function to approximate mortality rates at every age between 0 and 100
def mortality(x):
    # Define the age midpoints
    midpoints = np.asarray([0, 7, 17, 22, 27, 32, 37, 42, 47, 52, 57, 62, 67, 72, 77, 82, 93, 99])
...
# Approximate the COVID-19 mortality rate by month, race, gender and single years of age
for m in df_covid.month.unique():
    df_covid.loc[(df_covid.month == m) & (df_covid.race == 1) & (df_covid.latin == 0), 'M'] = mortality(df_group.loc[(df_group.month == m) & (df_group.race == 1) & (df_group.latin == 0), 'M'])
    df_covid.loc[(df_covid.month == m) & (df_covid.race == 2) & (df_covid.latin == 0), 'M'] = mortality(df_group.loc[(df_group.month == m) & (df_group.race == 2) & (df_group.latin == 0), 'M'])
    df_covid.loc[(df_covid.month == m) & (df_covid.race == -1) & (df_covid.latin == 1), 'M'] = mortality(df_group.loc[(df_group.month == m) & (df_group.race == -1) & (df_group.latin == 1), 'M'])
    df_covid.loc[(df_covid.month == m) & (df_covid.race == -1) & (df_covid.latin == -1), 'M'] = mortality(df_group.loc[(df_group.month == m) & (df_group.race == -1) & (df_group.latin == -1), 'M'])
...
# Merge with the actual number of COVID-19 deaths by race and age group
covid_monthly = pd.merge(df_covid, covid_monthly, how='left')

# Re-scale the approximated number of COVID-19 deaths by race and age group
covid_monthly.loc[:, 'rescale'] = covid_monthly.deaths.values / covid_monthly.groupby(['race', 'latin', 'agegroup', 'month'], as_index=False).deaths_approx.transform(lambda x: x.sum()).deaths_approx.values
covid_monthly = covid_monthly.drop(['deaths', 'agegroup', 'M'], axis=1)
covid_monthly.loc[:, 'deaths'] = covid_monthly.deaths_approx * covid_monthly.rescale
covid_monthly = covid_monthly.drop(['deaths_approx', 'rescale'], axis=1)

# Save the data
covid_monthly.to_csv(os.path.join(cdc_f_data, 'covid-19 monthly.csv'), index=False)
